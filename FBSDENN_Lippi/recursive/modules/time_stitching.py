...
def _init_quadratic_coupled_parameters(model, parameters):
    model.mu1 = parameters["mu1"]
    model.mu2 = parameters["mu2"]
    model.c1 = parameters["c1"]
    model.c2 = parameters["c2"]
    model.c3 = parameters["c3"]
    model.c4 = parameters["c4"]
    model.gamma = parameters["gamma"]
    model.s1 = parameters["s1"]
    model.s2 = parameters["s2"]
    model.s3 = parameters["s3"]
    model.x_max = parameters["x_max"]
    model.v_min = parameters["v_min"]
    model.v_max = parameters["v_max"]
    model.d = parameters["d"]
    model.const = parameters["const"]
...
class FBSNN_Recursive(FBSNN):
    ...
    def __init__(
        ...
        # Costanti TF create PRIMA di FBSNN.__init__ perché net_u viene usata nella build del grafo.
    ...
    def Dg_tf(self, X):
        if self.terminal_blob is None:
            return tf.gradients(self._model_g_tf(X), X)[0]
