...
class FBSNN(ABC): # Forward-Backward Stochastic Neural Network
    def __init__(self, Xi, T,
        ...
        #importo come parametri del modello
        self.activation = activation
        self.clip_grad_norm = clip_grad_norm
        self.use_antithetic_sampling = use_antithetic_sampling
        self.path_loss_weight = path_loss_weight
        self.terminal_y_loss_weight = terminal_y_loss_weight
        self.terminal_z_loss_weight = terminal_z_loss_weight
        ...
        #cambiamo il train_op per implementare il clipping
    ...
    def neural_net(self, X, weights, biases):
        ...
        for l in range(0,num_layers-2):
            W = weights[l]
            b = biases[l]
            pre_activation = tf.add(tf.matmul(H, W), b)
            if self.activation == "sin":
                H = tf.sin(pre_activation)
            elif self.activation == "tanh":
                H = tf.tanh(pre_activation)
            elif self.activation == "swish":
                H = pre_activation * tf.sigmoid(pre_activation)
            else:
                raise ValueError("Unsupported activation '{}'. Use: sin, tanh, swish".format(self.activation))
        W = weights[-1]
        b = biases[-1]
    ...
    def loss_function(self, t, W, Xi): # M x (N+1) x 1, M x (N+1) x D, 1 x D
        ...
        for n in range(0,self.N):
            ...
            loss += self.path_loss_weight * tf.reduce_mean(tf.square(Y1 - Y1_tilde))
